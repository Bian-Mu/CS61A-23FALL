...
def floor_div(args):
    ...
    if isinstance(first_arg, Pair):
        first_arg = calc_eval(first_arg)  # 对第一个参数进行计算

    second_arg = args.rest.first
    if isinstance(second_arg, Pair):
        second_arg = calc_eval(second_arg)  # 对第二个参数进行计算

    # 计算第一个和第二个参数的除法结果
    result = first_arg // second_arg

    # 检查是否还有更多的参数需要处理
    if args.rest.rest is not nil:
        # 如果还有更多参数，递归调用floor_div，将当前的结果和剩余的参数作为新的参数
