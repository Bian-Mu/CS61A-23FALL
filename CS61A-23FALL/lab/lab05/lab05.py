...
def distance(city_a, city_b):
    ...
    5.0
    """
    detx=abs(get_lon(city_a)-get_lon(city_b))
    dety=abs(get_lat(city_a)-get_lat(city_b))
    result=sqrt(detx**2+dety**2)
    return result
def closer_city(lat, lon, city_a, city_b):
    ...
    'Bucharest'
    """
    city_need=make_city(None, lat, lon)
    ka=distance(city_a,city_need)
    kb=distance(city_b,city_need)
    if ka>kb:
