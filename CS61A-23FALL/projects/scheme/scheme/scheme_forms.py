...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        func_params = signature.rest
        func_body = expressions.rest
        lambda_sentences = Pair('lambda', Pair(func_params, func_body))
        lambda_procedure = scheme_eval(lambda_sentences, env)
        ...
        env.define(func_name,lmbda_porcedure)
        return func_name
        """
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if not is_scheme_true(flag):
            return is_scheme_true(flag)
        else:
